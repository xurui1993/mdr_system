...
export function IssueOrderControlPanel({ theme, config, onChangeConfig, onCheckCookie }: IssueOrderControlPanelProps) {
 ...
 return (
 <div className="light:bg-white light:shadow-sm light:bg-white light:shadow-sm rounded-3xl cyber-border flex flex-col justify-between h-full p-5 relative group border light:border-slate-200 border-sky-500/10 shadow-[0_0_15px_rgba(14,165,233,0.1)]">
 ...
 <div className="flex flex-wrap items-center min-w-0 mb-5 gap-y-4 relative z-30 mt-1">
 {/* Left: City */}
 <div className="flex items-center shrink-0 min-w-0 pr-8">
 <span className="text-[13px] light:text-slate-700 text-slate-400 w-[80px] tracking-widest uppercase font-mono">{theme.lbl_city}</span>
 <div className="flex items-center rounded-xl focus-within:border-sky-500/50 transition-colors shadow-inner relative px-2 py-1.5 flex-wrap">
 {config.cities.map(c => (
 <div key={c} className="flex items-center group/item relative mr-1.5 mb-1.5">
 <button
 onClick={() => toggleIssueCity(c)}
 className={`px-4 py-1.5 text-[14px] transition-all rounded-lg font-medium tracking-wide ${(config.issueSelectedCities || []).includes(c) ? 'bg-sky-500/20 light:text-sky-700 text-sky-300 shadow-[inset_0_0_12px_rgba(14,165,233,0.3)]' : 'light:text-slate-700 text-slate-400 hover:text-sky-200 light:hover:bg-slate-200 hover:bg-slate-800/50'}`}
 >
 {c}
 </button>
 ...
 className="px-3 py-1.5 text-[16px] light:text-slate-600 text-slate-500 hover:light:text-sky-700 text-sky-300 hover:bg-sky-500/20 rounded font-medium transition-colors mb-1.5"
 title="新增"
 >
 +
 </button>
 ...
 {/* Right: Cycle */}
 <div className="flex items-center shrink-0 min-w-0 pr-8">
 <span className="text-[13px] light:text-slate-700 text-slate-400 w-[80px] tracking-widest uppercase font-mono">业务周期</span>
 ...
 <span className="text-[13px] light:text-slate-700 text-slate-400 w-[80px] shrink-0 tracking-widest font-mono mt-3 self-start flex flex-col gap-1">
 <span className="light:text-sky-600 text-sky-400">cookie</span>
 <span>验证</span>
 </span>
 ...
 className={` bg-slate-900/50 light:bg-slate-50 shadow-inner rounded-xl p-3 font-mono text-[13px] outline-none transition-all tracking-wide h-[90px] w-full resize-none overflow-y-auto leading-relaxed border ${
