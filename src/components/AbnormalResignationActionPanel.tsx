...
interface AbnormalResignationActionPanelProps {
  theme: Theme;
  isRunning: boolean;
  onRun: () => void;
  progress: number;
...
export function AbnormalResignationActionPanel({ theme, isRunning, onRun, progress }: AbnormalResignationActionPanelProps) {
  ...
  return (
    <div className="flex flex-col w-full h-auto justify-center px-4 py-2">
      <motion.button
        animate={{
          backgroundColor: isRunning ? "rgba(14, 165, 233, 0.15)" : "rgba(14, 165, 233, 0.05)",
          color: isRunning ? "#7dd3fc" : "#0ea5e9",
          borderColor: isRunning ? "rgba(14, 165, 233, 0.5)" : "rgba(14, 165, 233, 0.3)",
          boxShadow: isRunning ? "0 0 20px rgba(14, 165, 233, 0.3), inset 0 0 10px rgba(14, 165, 233, 0.2)" : "0 0 0px rgba(0,0,0,0)",
          scale: isRunning ? 0.98 : 1
        }}
        onClick={onRun}
        disabled={isRunning}
        className={`w-full h-[54px] rounded-xl border text-[16px] font-bold transition-all duration-300 flex items-center justify-center tracking-[0.2em] font-display backdrop-blur-sm ${isRunning ? 'cursor-not-allowed opacity-90' : 'hover:scale-[1.02] hover:bg-sky-500/20 hover:border-sky-500/50 hover:text-sky-300 hover:shadow-[0_0_20px_rgba(14,165,233,0.3),inset_0_0_10px_rgba(14,165,233,0.2)]'}`}
      >
        {isRunning ? '任务执行中' : progress >= 100 ? '操作成功' : '启动流程'}
      </motion.button>

      <div className="flex flex-col mt-6">
        <div className="flex justify-between items-end mb-4 px-1">
          <span className="text-[14px] text-sky-400 tracking-widest font-bold animate-breathe">{progress <= 0 && !isRunning ? '等待接入' : `[${progress}%] ${isRunning ? '运算中...' : progress >= 100 ? '操作成功' : ''}`}</span>
