...
export function DateRangeCyclePicker({ cycles, currentCycle, startDate, endDate, onChange, onUpdateCycles }: DateRangeCyclePickerProps) {
 ...
 className="flex items-center gap-3 px-4 py-2 border light:border-slate-200 border-sky-500/20 hover:border-sky-500/50 rounded-xl transition-all shadow-[inset_0_0_12px_rgba(14,165,233,0.1)] hover:shadow-[0_0_15px_rgba(56,189,248,0.2)] bg-slate-900/40"
 >
 <CalendarIcon className="w-5 h-5 light:text-sky-600 text-sky-400" />
 <span className="text-sky-100 text-[15px] font-mono tracking-wider flex items-center">
 {currentCycle !== '自定义' ? <span className="mr-3 light:text-sky-700 text-sky-300 font-medium px-2 py-0.5 bg-sky-500/20 rounded shadow-sm drop-shadow-md">{currentCycle}</span> : null}
 ...
 className={`w-full text-left px-3 py-2 text-[14px] transition-all rounded-lg font-medium tracking-wide ${currentCycle === c ? 'bg-sky-500/30 light:text-sky-800 text-white shadow-[inset_0_0_12px_rgba(14,165,233,0.5)] border border-sky-400/50' : 'light:text-slate-700 text-slate-300 hover:bg-sky-500/20 hover:light:text-slate-900 border border-transparent'}`}
 >
 {c}
 </button>
 ))}
 ...
 <button onClick={() => setCurrentMonth(new Date(currentMonth.getFullYear(), currentMonth.getMonth() - 1, 1))} className="p-1.5 hover:bg-sky-500/20 rounded-lg light:text-sky-700 text-sky-300 transition-colors">
 <ChevronLeft className="w-5 h-5" />
 </button>
 <div className="font-mono text-[15px] light:text-slate-900 text-white font-bold tracking-wider">
 ...
 className={`
 w-full p-1.5 text-[13px] rounded-lg transition-all font-mono aspect-square flex items-center justify-center
 ${isSel ? 'bg-sky-500 text-white shadow-[0_0_15px_rgba(14,165,233,0.8)] font-bold border border-sky-400' : 
 isR ? 'bg-sky-500/30 text-sky-100 light:text-sky-800 font-medium' : 'light:text-slate-700 text-slate-300 light:font-medium hover:bg-sky-500/20 hover:light:text-slate-900 border border-transparent'}
 `}
 >
 {d.getDate()}
 </button>
 ...
 <span>单日: <span className="light:text-sky-700 text-sky-300">{tempStart}</span></span>
 ) : (
 <>
 <span>起: <span className="light:text-sky-700 text-sky-300">{tempStart || '--'}</span></span>
 <span>-</span>
 <span>止: <span className="light:text-sky-700 text-sky-300">{tempEnd || '--'}</span></span>
 </>
 )}
 </div>
 <button 
