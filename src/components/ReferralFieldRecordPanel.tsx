...
interface ReferralFieldRecordPanelProps {
  theme: Theme;
  config: AppConfig;
  isRunning: boolean;
...
export function ReferralFieldRecordPanel({ theme, config, isRunning }: ReferralFieldRecordPanelProps) {
  ...
  const [data, setData] = useState([
    { id: 1, month: '2026-05', approvalNum: 'FD-202605-001', city: '广州', site: '越秀公园', agentName: '飞虎队', refereeName: '刘五', reward: 800.00, type: '地推', status: '已发放' },
    { id: 2, month: '2026-05', approvalNum: 'FD-202605-002', city: '上海', site: '虹桥核心', agentName: '战狼队', refereeName: '陈六', reward: 600.00, type: '地推', status: '待审核' },
  ]);
  ...
  const filteredData = useMemo(() => {
    return data.filter(item => {
      const matchSearch = item.agentName.includes(searchQuery) || item.refereeName.includes(searchQuery);
  ...
  const handleAdd = () => {
    const newRecord = {
      id: -1, month: '2026-06', approvalNum: 'FD-NEW', city: '未知', site: '未知', agentName: '', refereeName: '', reward: 0, type: '地推', status: '待审核'
    };
    setEditingId(-1);
  ...
  const handleExport = () => {
    const headers = ['核算月份', '核算单号', '城市', '站点', '代理商名称', '被推荐人姓名', '奖励金额', '状态'];
    const csvContent = [
      headers.join(','),
      ...data.map(item => [item.month, item.approvalNum, item.city, item.site, item.agentName, item.refereeName, item.reward, item.status].join(','))
    ].join('\n');
    
    const blob = new Blob(['\uFEFF' + csvContent], { type: 'text/csv;charset=utf-8;' });
    const link = document.createElement('a');
    link.href = URL.createObjectURL(blob);
    link.download = `地推发放记录_${Date.now()}.csv`;
    link.click();
  };
    ...
    reader.onload = (event) => {
      ...
      const newRecords = rows.slice(1).map((row, index) => {
        ...
        return {
          ...
          approvalNum: cols[1] || `FD-IMP-${Date.now().toString().slice(-4)}`,
          city: cols[2] || '未知',
          site: cols[3] || '未知',
          agentName: cols[4] || '未知',
      ...
      <div className="relative z-10 flex flex-col h-full gap-6">
        ...
        <div className="flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-[20px] font-bold text-sky-100 tracking-wide">地推发放记录</h2>
          </div>
          ...
          <table className="w-full text-left border-collapse text-[13px] whitespace-nowrap">
            <thead className="sticky top-0 bg-slate-800/90 backdrop-blur-md border-b border-white/10 z-10">
              <tr>
                ...
                <th className="px-4 py-4 font-medium text-slate-400">地推团队</th>
            ...
            <tbody className="divide-y divide-white/5 text-slate-300">
              {editingId === -1 && (
                <tr className="bg-sky-500/10">
                  ...
                  <td className="px-4 py-3"><input type="text" className="w-16 bg-slate-900/50 border border-sky-500/30 rounded px-2 py-1 text-sky-100 placeholder:text-slate-500" value={editForm.agentName} onChange={e => setEditForm({...editForm, agentName: e.target.value})} /></td>
                  ...
                  <tr key={row.id} className="hover:bg-sky-500/5 transition-colors">
                    {editingId === row.id ? (
                      <>
                        ...
                        <td className="px-4 py-3"><input type="text" className="w-16 bg-slate-900/50 border border-sky-500/30 rounded px-2 py-1 text-sky-100" value={editForm.agentName} onChange={e => setEditForm({...editForm, agentName: e.target.value})} /></td>
                      ...
                      <>
                        ...
                        <td className="px-4 py-4 font-medium text-sky-100">{row.agentName}</td>
