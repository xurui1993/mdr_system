...
interface SalaryReissueRecordPanelProps {
  theme: Theme;
  config: AppConfig;
  isRunning: boolean;
...
export function SalaryReissueRecordPanel({ theme, config, isRunning }: SalaryReissueRecordPanelProps) {
  ...
  const [data, setData] = useState([
    { id: 1, month: '2026-05', approvalNum: 'SR-202605-001', city: '深圳', site: '南山中心', riderId: 'R-10023', name: '张三', phone: '13800138000', orders: 0, amount: 250.00, cycle: '补发单', settlementType: '其他', type: '补发', status: '已补发' },
    { id: 2, month: '2026-05', approvalNum: 'SR-202605-002', city: '北京', site: '朝阳东', riderId: 'R-10088', name: '王安', phone: '13900139088', orders: 0, amount: 150.00, cycle: '补发单', settlementType: '其他', type: '补发', status: '待审核' },
  ]);
  ...
  const handleAdd = () => {
    const newRecord = {
      id: -1, month: '2026-06', approvalNum: 'SR-NEW', city: '未知', site: '未知', riderId: 'R-', name: '', phone: '', orders: 0, amount: 0, cycle: '补发单', settlementType: '其他', type: '补发', status: '待审核'
    };
    setEditingId(-1);
  ...
  const handleExport = () => {
    const headers = ['业务月份', '补发单号', '城市', '站点', '骑手ID', '骑手姓名', '补发金额', '补发类别', '状态'];
    const csvContent = [
      headers.join(','),
      ...data.map(item => [item.month, item.approvalNum, item.city, item.site, item.riderId, item.name, item.amount, item.type, item.status].join(','))
    ].join('\n');
    
    const blob = new Blob(['\uFEFF' + csvContent], { type: 'text/csv;charset=utf-8;' });
    const link = document.createElement('a');
    link.href = URL.createObjectURL(blob);
    link.download = `薪资补发记录_${Date.now()}.csv`;
    link.click();
  };
    ...
    reader.onload = (event) => {
      ...
      const newRecords = rows.slice(1).map((row, index) => {
        ...
        return {
          ...
          approvalNum: cols[1] || `SR-IMP-${Date.now().toString().slice(-4)}`,
          city: cols[2] || '未知',
          site: cols[3] || '未知',
          riderId: cols[4] || 'R-00000',
          name: cols[5] || '未知',
          orders: 0,
          amount: Number(cols[6]) || 0,
          cycle: '补发单',
          settlementType: '其他',
          type: cols[7] || '补发',
          status: cols[8] || '待审核'
      ...
      <div className="relative z-10 flex flex-col h-full gap-6">
        ...
        <div className="flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-[20px] font-bold text-sky-100 tracking-wide">薪资补发记录</h2>
          </div>
        ...
        <div className="flex-1 overflow-auto rounded-xl border border-white/5 bg-slate-900/40 backdrop-blur-sm">
          <table className="w-full text-left border-collapse text-[13px] whitespace-nowrap">
            <thead className="sticky top-0 bg-slate-800/90 backdrop-blur-md border-b border-white/10 z-10">
              <tr>
                <th className="px-4 py-4 font-medium text-slate-400">补发月份</th>
                <th className="px-4 py-4 font-medium text-slate-400">补发审批编号</th>
                ...
                <th className="px-4 py-4 font-medium text-slate-400 text-right">补发金额(元)</th>
                <th className="px-4 py-4 font-medium text-slate-400">补发类别</th>
            ...
            <tbody className="divide-y divide-white/5 text-slate-300">
              {editingId === -1 && (
                <tr className="bg-sky-500/10">
                  ...
                  <td className="px-4 py-3"><input type="text" className="w-16 bg-slate-900/50 border border-sky-500/30 rounded px-2 py-1 text-sky-100 placeholder:text-slate-500" value={editForm.type} onChange={e => setEditForm({...editForm, type: e.target.value})} /></td>
                  ...
                  <tr key={row.id} className="hover:bg-sky-500/5 transition-colors">
                    {editingId === row.id ? (
                      <>
                        ...
                        <td className="px-4 py-3"><input type="text" className="w-16 bg-slate-900/50 border border-sky-500/30 rounded px-2 py-1 text-sky-100" value={editForm.type} onChange={e => setEditForm({...editForm, type: e.target.value})} /></td>
                      ...
                      <>
                        ...
                        <td className="px-4 py-4 text-slate-300">{row.type}</td>
                        <td className="px-4 py-4 flex justify-center">
                          <span className={`inline-flex items-center px-2 py-1 rounded-md text-[11px] font-medium border ${
                            row.status === '已补发' ? 'bg-sky-500/10 text-sky-400 border-sky-500/20' :
