...
interface SalaryBindActionPanelProps {
  theme: Theme;
  isRunning: boolean;
  onRun: () => void;
  progress: number;
...
export function SalaryBindActionPanel({ theme, isRunning, onRun, progress }: SalaryBindActionPanelProps) {
  ...
  return (
    <div className="flex flex-col w-full h-auto justify-center px-4 py-2">
      <motion.button
        ...
        onClick={onRun}
        disabled={isRunning}
        className={`w-full h-[54px] rounded-xl border text-[16px] font-bold transition-all duration-300 flex items-center justify-center tracking-[0.2em] font-display backdrop-blur-sm ${isRunning ? 'cursor-not-allowed opacity-90' : 'hover:scale-[1.02] hover:bg-sky-500/30'}`}
      >
        {isRunning ? '任务执行中' : progress >= 100 ? '操作成功' : '启动绑定任务'}
      </motion.button>
